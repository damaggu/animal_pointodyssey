...
def write_palette_png(data: np.array, filename: PathLike,
  ...
  palette[-2] = np.array([0, 0, 0]).astype(np.uint8)  # set alpha to 0 for background
  w = png.Writer(width=width, height=height, palette=palette, bitdepth=8)
  with open(filename, "wb") as fp:
    w.write(fp, data[:, :, 0])


def write_scaled_png(data: np.array, filename: PathLike, min: float = None, max: float = None) -> Dict[str, float]:
...
def multi_write_image(data: np.ndarray, path_template: str, write_fn=write_png,
                      max_write_threads=16, frame_idx=0, **kwargs):
  ...
  with multiprocessing.pool.ThreadPool(num_threads) as pool:
    args = [(img, path_template.format(i+frame_idx)) for i, img in enumerate(data)]
...
def write_rgb_batch(data, directory, file_template="rgb_{:05d}.png", max_write_threads=16, frame_idx=0):
  assert data.ndim == 4 and data.shape[-1] == 3, data.shape
  path_template = str(as_path(directory) / file_template)
  multi_write_image(data, path_template, write_fn=write_png, max_write_threads=max_write_threads, frame_idx=frame_idx)


def write_rgba_batch(data, directory, file_template="rgba_{:05d}.png", max_write_threads=16, frame_idx=0):
  assert data.ndim == 4 and data.shape[-1] == 4, data.shape
  path_template = str(as_path(directory) / file_template)
  multi_write_image(data, path_template, write_fn=write_png, max_write_threads=max_write_threads, frame_idx=frame_idx)


def write_uv_batch(data, directory, file_template="uv_{:05d}.png", max_write_threads=16, frame_idx=0):
  assert data.ndim == 4 and data.shape[-1] == 3, data.shape
  path_template = str(as_path(directory) / file_template)
  multi_write_image(data, path_template, write_fn=write_png, max_write_threads=max_write_threads, frame_idx=frame_idx)


def write_normal_batch(data, directory, file_template="normal_{:05d}.png", max_write_threads=16, frame_idx=0):
  assert data.ndim == 4 and data.shape[-1] == 3, data.shape
  path_template = str(as_path(directory) / file_template)
  multi_write_image(data, path_template, write_fn=write_png, max_write_threads=max_write_threads, frame_idx=frame_idx)
...
def write_coordinates_batch(data, directory, file_template="object_coordinates_{:05d}.png",
                            max_write_threads=16, frame_idx=0):
  assert data.ndim == 4 and data.shape[-1] == 3, data.shape
  path_template = str(as_path(directory) / file_template)
  multi_write_image(data, path_template, write_fn=write_png, max_write_threads=max_write_threads, frame_idx=frame_idx)


def write_depth_batch(data, directory, file_template="depth_{:05d}.tiff", max_write_threads=16, frame_idx=0):
  assert data.ndim == 4 and data.shape[-1] == 1, data.shape
  path_template = str(as_path(directory) / file_template)
  path_visualize_template = str(as_path(directory) / (file_template[:-5] + '.png'))
  multi_write_image(data, path_template, write_fn=write_tiff, max_write_threads=max_write_threads, frame_idx=frame_idx)

  min = np.min(data)
  data[np.where(data > DEPTH_MAX)] = 0
  max = np.max(data)
  print("depth range: {} to {}".format(min, max))
  multi_write_image(data, path_visualize_template, write_fn=write_scaled_png, max_write_threads=max_write_threads, min=min, max=max,
                    frame_idx=frame_idx)
...
def write_segmentation_batch(data, directory, file_template="segmentation_{:05d}.png",
  ...
  try:
    multi_write_image(data, path_template, write_fn=write_palette_png,
                      max_write_threads=max_write_threads, palette=palette, frame_idx=frame_idx)
  except Exception as e:
    logger.warning("Exception while writing segmentation: %s, skip", e)


def write_flow_batch(data, directory, file_template="flow_{:05d}.png", name="flow",
                     max_write_threads=16, range_file="data_ranges.json", frame_idx=0):
