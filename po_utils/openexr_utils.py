...
def replace_cryptomatte_hashes_by_asset_index(
  ...
  Args:
    ...
    assets: List of data to use for replacement.
  """
...
def process_depth(exr_layers, cam_info):
  # blender_assets returns z values (distance to camera plane)
...
def process_segementation(exr_layers, scene_info):  # pylint: disable=unused-argument
  ...
  return replace_cryptomatte_hashes_by_asset_index(
      exr_layers["segmentation_indices"][:, :, :1], scene_info['data'])
...
def postprocess(
    ...
    scene_info['data'] += ['Plane']
    #scene_info['data'] = ['background'] + [i for i in scene_info['data'] if 'robot' in i or 'Rock' in i or 'Cube' in i] # + ['Sphere'] # The last one is the background
