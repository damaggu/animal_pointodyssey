...
def fix_annotations(file_path):
    fix_script = f"python -m po_utils.gen_tracking_indoor --data_root {file_path}  --cp_root {file_path} --sampling_scene_num 20000 --sampling_character_num 5000"
...
def add_videos(path):
    ...
    for d in dir_list:
        ...
        if "kubric.npy" in os.listdir(full_path):
            ...
            shutil.copy(os.path.join(full_path, "kubric.npy"), os.path.join(output_path, "kubric.npy"))
        cur += 1


paths = ["results/mouse_07-28T18:08:01"]
for p in paths:
    add_videos(p)