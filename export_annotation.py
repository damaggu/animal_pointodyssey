...
if __name__ == '__main__':
    ...
    parser.add_argument('--background_hdr_path', type=str,
                        default='./data/hdri/countrytrax_midday_4k.exr')
    ...
    if args.exr:
        exr_script = 'python -m po_utils.openexr_utils --data_dir {} --output_dir {} --batch_size {} --frame_idx {}'.format(
            args.save_dir, args.save_dir + '/exr_img', args.batch_size, args.frame_idx)
        os.system(exr_script)
    if args.export_obj:
        obj_script = 'blender --background --python {}/po_utils/export_scene.py \
    ...
    if args.export_tracking:
        tracking_script = 'python -m po_utils.gen_tracking_indoor --data_root {} --cp_root {} --sampling_scene_num {} --sampling_character_num {}'.format(
